#!/usr/bin/env node
/**
 * Remove exports with zero consumers.
 *
 * Usage:
 *   node build/clean-orphans.mjs           # report only (exit 1 if orphans found)
 *   node build/clean-orphans.mjs --fix     # auto-remove orphan exports
 *   node build/clean-orphans.mjs --json    # machine-readable output
 */
...
function extractOrphanExports(report) {
...
function extractOrphanTypes(report) {
	if (!report || !report.issues) return [];
...
await runMain(async () => {
	log.title('Langflower — clean orphan exports');
	...
	if (exitCode !== 0 && exitCode !== 1) {
		...
		throw new BuildError('clean-orphans', 'knip execution failed', {
			exitCode,
		});
	}
	...
	if (!report) {
		...
		throw new BuildError('clean-orphans', 'knip output parse failed', {});
	}

	let orphanExports = extractOrphanExports(report);
	let orphanTypes = extractOrphanTypes(report);

	// Filter to package files only
	orphanExports = orphanExports.filter((e) => isPackageFile(e.file));
	orphanTypes = orphanTypes.filter((t) => isPackageFile(t.file));

	// Filter known false positives (re-exports through intermediate barrels)
	orphanExports = orphanExports.filter((e) => !isKnownReExport(e));

	const all = [...orphanExports, ...orphanTypes];
	const formatted = {
		total: all.length,
