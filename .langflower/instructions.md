...
`tsconfig.json` (`hello-embed`, `lf-dev-nodes`). Without it the pack does not
compile. `my-nodes` uses extensionless imports and does not need the flag.
...
- `lf-dev-nodes/` — dogfood pack: `lf-dev-tools` (npm script ToolHandles +
  `run_targeted_tests`) and `lf-review-gate` (format then typecheck → test;
  format never fails the gate; `ok` passthroughs `trigger`). Not a bootstrap
  seed.
