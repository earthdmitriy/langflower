...
export const BRIDGE_TOOL_META = {
	...
	'runner.askUser.reply': {
		description:
			'Operator text for a runtime `ask_user` builtin wait inside the tool loop (not a canvas HITL port — see `runner.askUser.ask`).',
		typeExpr: 'RunnerAskUserReplyPayload',
		inputSchema: {
			type: 'object',
			properties: {
				runId: {
	...
	'runner.checkpoint.discard.requested': {
		...
		inputSchema: {
			type: 'object',
			description:
				'TypeScript payload: RunnerCheckpointDiscardRequestedPayload',
			additionalProperties: true,
		},
	},
	'runner.started': {
	...
	'runner.askUser.ask': {
		description:
			'Runtime `ask_user` wait for operator text (feed + composer textarea). Stays inside the internal tool loop — not a graph HITL edge.',
		typeExpr: 'RunnerAskUserAskPayload',
		inputSchema: {
			type: 'object',
			description: 'TypeScript payload: RunnerAskUserAskPayload',
			additionalProperties: true,
		},
	},
	'runner.askUser.accepted': {
		description:
			'Server accepted one text reply for a still-pending `ask_user`. Every client removes that ask from its UI only on this fact.',
		typeExpr: 'RunnerAskUserReplyPayload',
		inputSchema: {
			type: 'object',
			properties: {
	...
	'langflower.secrets.save.requested': {
		description:
			'Persist named KV secrets to the user-global secrets file, then broadcast {@link LangflowerConfigSnapshotPayload} (`secretIds` + `secretsPath` only). JSONL logs this type as `"REDACTED"`.',
		typeExpr: 'LangflowerSecretsSaveRequestedPayload',
		inputSchema: {
			type: 'object',
			description:
				'TypeScript payload: LangflowerSecretsSaveRequestedPayload',
			additionalProperties: true,
		},
	},
