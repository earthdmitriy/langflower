...
describe('common-mcp-stdio connect errors (S5)', () => {
	...
	it('emits output-emitted error when connect fails', async () => {
		...
		expect(String(failed[3].error)).toMatch(/MCP stdio connect failed/);
		expect(String(failed[3].error)).toContain('mcp-1');
		expect(String(failed[3].error)).toContain('spawn ENOENT');
